...
void ADIS16448_IMU::Calibrate() {
    ...
    while (timer.Get() < 5)
    {
        m_accel_offset_x += GetAccelX();
        m_accel_offset_y += GetAccelY();
        m_accel_offset_z += GetAccelZ();
        count ++;
    }
    m_accel_offset_x = 0;//= count;
    m_accel_offset_y = 0;//= count;
    m_accel_offset_z = 0;//= count;
...
double ADIS16448_IMU::GetAccelX() const {
    std::lock_guard<priority_mutex> sync(m_mutex);
    return m_accel_x - m_accel_offset_x;
}

double ADIS16448_IMU::GetAccelY() const {
    std::lock_guard<priority_mutex> sync(m_mutex);
    return m_accel_y - m_accel_offset_y;
}

double ADIS16448_IMU::GetAccelZ() const {
    std::lock_guard<priority_mutex> sync(m_mutex);
    return m_accel_z - m_accel_offset_z;
}
